...
def insert_records(cursor, records, batch=20):
    records_to_insert = [
        (rec['section'], rec['subsection'], rec['question'], rec['answer'], hash_text(rec['text']))
    ...
    for i in range(0, len(records), batch):
        execute_values(cursor,
                       ...
                       "VALUES %s RETURNING id", records_to_insert[i: i + batch])
...
def update_record(cursor, rec, doc_id):
    cursor.execute("""
        ...
        WHERE id = %s
    """, (rec['section'], rec['subsection'], rec['answer'], rec['question'], hash_text((rec['text'])), doc_id))
